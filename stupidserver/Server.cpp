...
void Server::SetupServer() {
	static const std::filesystem::path currentPath =
		std::filesystem::relative("stupidserver");
	...
	mServerSocket = socket(serverResult->ai_family, serverResult->ai_socktype,
		serverResult->ai_protocol);
	if (mServerSocket == INVALID_SOCKET) {
		std::cerr << "creating socket failed with " << WSAGetLastError();
	}

	if (ioctlsocket(mServerSocket, FIONBIO, &uNonBlockingMode) != 0)
		ERR("ioctlsocket");

	mIResult =
		bind(mServerSocket, serverResult->ai_addr, (int)serverResult->ai_addrlen);
		...
		if (pollResult) {
			if (events[0].revents & POLLIN) {
				...
				if (event_count < MAX_EVENTS + 1) {
					...
					send(mClientSocket, "Server full", sizeof("Server full"), 0);
				} // Sorry I couldn't bother
			}
			for (int i = 1; i <= MAX_EVENTS; i++) {
				if (events[i].fd != 0 && events[i].revents & POLLIN) {
					...
					if (send(events[i].fd, buffer.c_str(), buffer.size(), 0) ==
						SOCKET_ERROR)
						ERR("send");
					shutdown(events[i].fd, SD_SEND);
					...
					if (mBytesReceived <= 0) {
						if (shutdown(events[i].fd, SD_SEND) != 0)
							ERR("shutdown");
						closesocket(events[i].fd);
						events[i].fd = 0;
						events[i].events = 0;
